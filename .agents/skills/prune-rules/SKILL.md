...
sitting in `architecture.md`. Nested `AGENTS.md` files are **path-scoped**: an agent working
in `src/core/` never loads `src/features/AGENTS.md`. A rule that must hold everywhere belongs
in `architecture.md` or root `AGENTS.md`, or it is not enforced where it matters.
