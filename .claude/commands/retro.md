...
1. **Decide if it's worth saving.** Keep only what would have saved real time had you known
   it up front. Skip anything the repo already records (code, git history, `AGENTS.md`,
   `docs/architecture.md`, the nested `AGENTS.md` files) or that mattered only to this task.
...
4. **Escalate if it's recurring or procedural.** A one-off fact stays a memory. But if the
   ...
   - a repeatable workflow → a new command in `.claude/commands/` or skill in
     `.agents/skills/`;
   - a durable standard → an edit to `AGENTS.md`, `docs/architecture.md`, or the nested
     `AGENTS.md` for the layer it governs;
