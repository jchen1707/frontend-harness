...
is `$CLAUDE_VAULT_DIR`, or the parent of `$CLAUDE_LEARNINGS_DIR` when that is unset (in
PowerShell, `$env:CLAUDE_VAULT_DIR`).
...
5. **Check the repo too.** `CLAUDE.md`, `docs/architecture.md`, the nested `CLAUDE.md` files
...
an edit to `CLAUDE.md`, `docs/architecture.md`, or the nested `CLAUDE.md` for the layer it
