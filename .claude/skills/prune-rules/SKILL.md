...
This skill audits them and proposes repairs. The focus is `$ARGUMENTS`; if empty, audit all
of them.
...
**Wrong altitude.** A cross-layer invariant sitting in a leaf file, or a single-layer detail
sitting in `architecture.md`. Nested `CLAUDE.md` files are **path-scoped**: an agent working
in `src/core/` never loads `src/features/CLAUDE.md`. A rule that must hold everywhere belongs
in `architecture.md` or root `CLAUDE.md`, or it is not enforced where it matters.

**Dead rules.** A rule about a library, a command or a file that no longer exists. Verify
before deleting: read `package.json`, run the command, look for the path.
...
**Tooling overlap.** A rule that ESLint, Prettier or `tsc` already enforces. Delete it. A rule
the tooling enforces is noise in a prompt, and it dilutes the rules that need a human to
follow them.

**Rules the tooling only appears to enforce.** The opposite failure, and worse. Check that
each machine-enforced claim is actually machine-enforced — `eslint-plugin-boundaries` fails
open, so a `src/**` shape no element pattern classifies is silently unchecked while the rule
reads as green. A prose rule is the right repair for what the config cannot see.

**Unverifiable rules.** "Write clean components." "Be thoughtful." They cost context and
change no behaviour. Replace with something checkable or cut.

**Missing rationale.** A rule with no reason is a rule that gets deleted by the next person
who finds it inconvenient. If the reason is not obvious, add one line. If nobody can remember
the reason, that is a candidate for deletion.
...
2. **Verify each claim you doubt** against the repo. Do not report a rule as dead because you
   do not recognise it — check `package.json`, the file tree, the command's `--help`.
...
```markdown
> **Deprecated 2026-08-06.** Use the `useQuery` wrapper in the slice's `services/`. The old
> form bypasses the shared error mapping. This note is removed once no call site uses it.
```
...
repair, and the reason. End with what you checked and found healthy — a clean audit is a
useful result and should be said plainly, not padded into findings.
