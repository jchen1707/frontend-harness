...
step 1 (gather context) to check whether a new request was already turned down, so a rejection
doesn't get re-litigated from scratch each time it resurfaces.
...
- **Kebab-case concept names**: `dark-mode.md`, `offline-mode.md`, `server-rendering.md`.
  Recognisable without opening the file.
- **Only rejected enhancements land here.** Not bugs, and _not_ things closed as `wontfix`
...
- **Reasons must be durable.** Scope, philosophy, or a technical constraint — not "we're busy
  right now", which is a deferral, not a rejection.
...
The design system defines one palette in `tailwind.config.js`, and every primitive in
`src/components/ui` reads it directly. Supporting a second theme would need a theme context,
per-component token resolution, and a persistence layer for the preference — a significant
