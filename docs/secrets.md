...
| `protect_paths.mjs` (hook) | Refuses `Read` **and** write on `.env` and `.env.*`, and refuses the shell commands that reach a secret without naming a file — environment dumps, single-variable expansions, inline interpreters. Permits `.env.example`, the committed template. These rules are built into the shared hook and are not in `harness.config.json`: a guard whose config goes missing and quietly protects nothing is worse than no guard. |
