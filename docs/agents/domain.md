...
`src/features/CLAUDE.md`. They live in `src/core/`, `src/components/`, `src/features/` and
...
cross-slice only through `index.ts` — and the approved stack. Neither changes without a
`CLAUDE.md` edit first.
